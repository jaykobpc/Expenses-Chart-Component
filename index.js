    ...
    json.forEach((item) => {
      const result = `
        <div data-bar="${item.amount}%" class="graphbox">
            <span class="graphbox__tooltip" role="tooltip">${item.amount}%</span>
            <span style="height: ${item.amount}%" class="graphbox__bar ${item.amount > 50 ? 'color-cyan' : ''}"></span>
            <span class="graphbox__text">${item.day}</span>
        </div>
        `;

      GraphWrapper.insertAdjacentHTML("beforeend", result);
      GraphBar = document.querySelectorAll(".graphbox__bar");
    });
